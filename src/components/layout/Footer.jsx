...
const Footer = () => (
  <footer className="w-full bg-[#f5f5f5] dark:bg-secondary">
    <div className="mx-auto lg:px-24 px-4 pt-10">
      <div className="grid grid-cols-2 pb-8 lg:grid-cols-5 gap-8">
        <div className=" col-span-2 text-left">
          ...
          <p className="py-4 pr-8 text-sm text-gray-500 dark:text-gray-50">
            We at Inspirigence Advisors having expertise in Fund Accounting and Fund administration services, Regulatory compliances, Accounting services, CFO/COO functions, Investor Relationship and Advisory to various entities like Private Equity/Hedge Funds, other business entities, and corporates.
          </p>
          <Link
              href="/contact"
              className="py-2.5 px-5 h-9 block w-fit bg-primary hover:scale-95 rounded-full shadow-sm text-xs text-white  transition-all lg:mx-0"
            >
              Contact us
            </Link>
        </div>
          ...
          <div key={index} className="text-left col-span-2 lg:col-span-1">
            ...
            <ul className="text-sm space-y-2">
              {section.links.map((item, i) => (
                <li key={i}>
                  <Link href={item.href} className="text-secondary hover:text-primary hover:dark:text-primary dark:text-white ">{item.label}</Link>
                </li>
              ))}
            </ul>
          </div>
